// Final test to verify our solution handles the specific user issue
...
const testLine = "- نرى فراغ مضيء كألف شمس ساطعة...";

console.log("Testing the specific user issue...");
console.log(`Line: ${testLine}`);
console.log(`Is likely action: ${ScreenplayClassifier.isLikelyAction(testLine)}`);

// This should return true (action) instead of false (dialogue)
if (ScreenplayClassifier.isLikelyAction(testLine)) {
  console.log("✅ SUCCESS: The line is correctly classified as action!");
} else {
  console.log("❌ FAILURE: The line is incorrectly classified!");
}